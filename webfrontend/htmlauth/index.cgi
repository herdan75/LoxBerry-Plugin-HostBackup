...
sub action_info {
  ...
  my %help = (
    ...
    'maintenance-settings' => ['Erweiterte Aufbewahrung und Integritätsprüfung', "Optionaler Bereich für Aufbewahrung und Datenprüfung. Standard bleibt die eingestellte Anzahl Backups. Alternativ kannst du Tages-, Wochen- und Monatsstände behalten sowie Fristen für Task-Logs und Import-Quarantäne festlegen. Geschützte Backups und die jüngste brauchbare Sicherung bleiben erhalten.\n\nDie regelmässige Integritätsprüfung ist standardmässig aus. Sie prüft gesicherte Daten, benötigt Zeit und zusätzliche Lesezugriffe. Fehlt eine benötigte Prüfsummen-Vergleichsbasis, wird diese zuerst angelegt; das ist noch kein erfolgreicher Vergleich. Eine erfolgreiche Prüfung ersetzt keinen Restore-Test.\n\nÄnderungen zuerst speichern. Die Löschvorschau zeigt den gespeicherten Stand und löscht selbst nichts. Eine manuelle Bereinigung muss separat bestätigt werden; nach erfolgreichen Backups gilt die gespeicherte Aufbewahrungsregel automatisch. Das Öffnen dieser Hilfe ändert keine Einstellungen und startet keine Aktion."],
...
my %action_help = map { $_ => action_info($_) } qw(
  ...
  maintenance-settings maintenance-preview restore-files restore-destination restore-volumes restore-preview restore-start
);
...
<summary>Erweiterte Aufbewahrung und Integritätsprüfung $action_help{'maintenance-settings'}</summary>
